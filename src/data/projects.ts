...
export const projects: Project[] = [
  {
    ...
    overview:
      "状態管理、条件分岐、設計、テストなど幅広い実装を経験するため、ゲームを題材に開発している個人開発プロジェクトです。Claude CodeとCodexを活用しつつ、仕様整理、設計判断、レビューは自分で行っています。",
    purpose:
    ...
    features: [
      ...
      "AIが生成した実装を自分でレビューし、必要に応じて修正する開発フロー",
    ],
    responsibilities: [
      "仕様整理と設計判断",
    ...
    solutions: [
      ...
      "AIの提案内容を自分で確認し、仕様と異なる変更が含まれていないかレビューする",
      "小さな単位で実装と検証を行い、問題があれば早い段階で修正する",
    ],
    quality: [
      "単体テストを用いた主要ロジックの検証",
      "実際のプレイによるUIとゲームルールの確認",
      "AIが生成したコードのレビューと必要に応じた修正",
      "ビルドやテスト結果の確認",
    ],
    futurePlans: [
      "READMEと公開情報の整理",
